# 📱 안드로이드 서버 모니터링 앱
# 설치 필요: pip install kivy kivymd requests plyer
...
DEFAULT_CONFIG = {
    "servers": [
        {
            "name": "1300빅터 (111.111.111.65)",
            "url": "http://kotheday.duckdns.org:80",
            "monitor": True,
            "status": "알 수 없음"
        },
        {
            "name": "12700h빅터 (111.111.111.53)",
            "url": "http://kotheday.duckdns.org:776",
            "monitor": True,
            "status": "알 수 없음"
        },
        {
            "name": "12700h_asusu (111.111.111.72)",
            "url": "http://kotheday.duckdns.org:775",
            "monitor": True,
            "status": "알 수 없음"
        },
        {
            "name": "12700h_johyun (111.111.111.48)",
            "url": "http://kotheday.duckdns.org:774",
            "monitor": True,
            "status": "알 수 없음"
        }
    ],
    "check_interval": 5,  # 안드로이드에서는 5초로 설정
    "alert_interval": 60,  # 1분마다 알림
...
class ServerMonitorApp(MDApp):
        ...
        while self.monitoring_active:
            try:
                ...
                for i, server in enumerate(self.server_list):
                    ...
                    # 오프라인 상태 지속 알림
                    elif (new_status != "온라인" and 
                          current_time - self.last_alert_times[i] >= self.alert_interval):
                        
                        self.send_persistent_notification(server['name'], new_status)
                        self.last_alert_times[i] = current_time
    ...
    def send_persistent_notification(self, server_name, status):
        """지속적인 오프라인 알림"""
        if not self.notification_enabled:
            return
        ...
        try:
            ...
            print(f"[지속 알림] {server_name} - {status}")
